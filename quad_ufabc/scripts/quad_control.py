...
class Controller:
    ...
    # --------------------------- Termos para a utilização do controle ótimo ----------------------- #
    # ------------------------ Modelo Linear para a dinâmica rotacional do Drone ------------------- #
    # Sistema linear: dx = Ax + Bu
    # Matriz A (Modelo mais simples)
    A = np.zeros((6,6))
    A[0, 3] = 1
    A[1, 4] = 1
    A[2, 5] = 1
    # Matriz B (Modelo mais simples)
    B = np.zeros((6, 3))
    B[3, 0] = 1/Ixx
    B[4, 1] = 1/Iyy
    B[5, 2] = 1/Izz
    ...
    # Resolve a equação algébrica de Riccati contínua para encontrar a matriz P (abordagem mais simples)
    P = solve_lqr(A, B, Q, R)
 ...
 ############################### PEDRO: LQR AND PID CONTROL APPROACH USING EULER ANGLES ##########################
    def att_control_LQR(self, ang_atual, ang_vel_atual, ang_des):
        """
    ...
    def pos_control_PID(self, pos_atual, pos_des, vel_atual, vel_des, accel_des, psi):
        ...
        Kp = np.array([[1.7010020000861685, 0 ,0],
                       [0,1.7010020000861685, 0],
                       [0, 0, 0.9527546134347671]])
        Ki = np.array([[1.3137332537803728, 0, 0],
                       [0,1.3137332537803728, 0],
                       [0, 0, 0.2750127680608602]])
        Kd = np.array([[1.140529531568228, 0, 0],
                       [0,1.1405295315682282, 0],
                       [0, 0, 1.442]])
    ...
    def att_control_PD2(self, ang_atual, ang_vel_atual, ang_des):
        
        phi = float(ang_atual[0])
        theta = float(ang_atual[1])
        psi = float(ang_atual[2])

        p = float(ang_vel_atual[0])
